  ...
  return (
    <>
      ...
      <header className="max-w-3xl mx-auto px-4 pt-16 pb-10 border-b border-[#1a1a1a]">
        <p className="text-[10px] uppercase tracking-widest text-[#555] mb-6">Generation IV</p>
        <p className="font-arabic text-white mb-6 text-right" style={{ fontSize: "36px", lineHeight: "2.8" }} dir="rtl">
          تَبَع التَّابِعِين
        </p>
        <h1 className="text-base font-semibold text-white mb-3">Taba Tabi&apos;een</h1>
      ...
      <section className="max-w-3xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 gap-px bg-[#1a1a1a] border border-[#1a1a1a]">
          {people.map((p) => {
            ...
            return (
              <Link key={p.id} href={`/taba-tabieen/${p.slug}`} className="bg-black p-6 group hover:bg-[#080808] transition-colors flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-[10px] uppercase tracking-widest text-[#555] mb-3">{p.years}</p>
                  <p className="text-base font-semibold text-white mb-1 group-hover:text-[#ccc] transition-colors">{p.name}</p>
