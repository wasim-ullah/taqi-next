    ...
    <>
      ...
      <section className="max-w-5xl mx-auto px-4 pt-20 pb-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-[#e5e5e5]">
          <div className="bg-white p-8 flex flex-col justify-between min-h-[320px]">
            <div>
              <p className="text-[10px] uppercase tracking-widest text-[#777] mb-8">
                taqi &middot; the pure one
              </p>
              <h1 className="text-sm font-semibold text-[#111] mb-4 leading-relaxed">
                Live as they lived.
              </h1>
            ...
            <div className="mt-10 flex gap-px">
              <Link href="/prophet" className="bg-black text-white text-xs font-semibold uppercase tracking-widest px-5 py-2.5 hover:bg-[#333] transition-colors">
                Begin reading
              </Link>
              <Link href="/about" className="border border-[#ccc] text-[#888] text-xs uppercase tracking-widest px-5 py-2.5 hover:border-[#aaa] hover:text-[#111] transition-colors">
                About
              </Link>
            </div>
          </div>

          <div className="bg-white p-8 flex flex-col items-end justify-center">
            <p className="text-[10px] uppercase tracking-widest text-[#999] mb-6 self-start">
              Al-Ahzab &middot; 33:21
            </p>
            <p className="font-arabic text-[#111] text-right" style={{fontSize:"30px",lineHeight:"2.8"}} dir="rtl">
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        <div className="mb-8 flex items-center justify-between border-b border-[#e5e5e5] pb-4">
          ...
          <p className="text-[10px] uppercase tracking-widest text-[#aaa]">&#x627;&#x644;&#x623;&#x62C;&#x64A;&#x627;&#x644; &#x627;&#x644;&#x623;&#x631;&#x628;&#x639;&#x629;</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-[#e5e5e5] border border-[#e5e5e5]">
          {generations.map((g) => (
            <Link key={g.num} href={g.href} className="bg-white p-8 flex flex-col justify-between min-h-[240px] group hover:bg-[#f5f5f5] transition-colors">
              <div>
                <div className="flex items-start justify-between mb-6">
                  <p className="text-[10px] uppercase tracking-widest text-[#999]">Generation {g.num} &middot; {g.period}</p>
                  <span className="text-[10px] text-[#aaa] uppercase tracking-widest">{g.count}</span>
                </div>
                <p className="font-arabic text-[#aaa] mb-4 text-right" style={{fontSize:"26px",lineHeight:"2.6"}} dir="rtl">{g.arabic}</p>
                <p className="text-sm font-semibold text-[#111] mb-3 group-hover:text-[#444] transition-colors">{g.title}</p>
                <p className="text-sm text-[#888] leading-relaxed">{g.desc}</p>
              </div>
              <p className="text-[10px] uppercase tracking-widest text-[#999] mt-6 group-hover:text-[#888] transition-colors">Read &rarr;</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* Six pillars */}
      <section className="max-w-5xl mx-auto px-4 py-10 border-t border-[#e5e5e5]">
        <div className="mb-8 flex items-center justify-between border-b border-[#e5e5e5] pb-4">
          <p className="text-[10px] uppercase tracking-widest text-[#777]">Six Domains of Practice</p>
          <p className="text-[10px] uppercase tracking-widest text-[#aaa]">&#x633;&#x62A;&#x629; &#x645;&#x62C;&#x627;&#x644;&#x627;&#x62A;</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-[#e5e5e5] border border-[#e5e5e5]">
          {pillars.map((p) => (
            <Link key={p.id} href={`/prophet?topic=${p.id}`} className="bg-white p-6 group hover:bg-[#f5f5f5] transition-colors">
              <p className="font-arabic text-[#aaa] mb-3 text-right group-hover:text-[#888] transition-colors" style={{fontSize:"22px",lineHeight:"2.4"}} dir="rtl">{p.arabic}</p>
              <p className="text-xs font-semibold text-[#aaa] mb-1 group-hover:text-[#111] transition-colors">{p.label}</p>
              <p className="text-[10px] text-[#999] leading-relaxed">{p.desc}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* About */}
      <section className="max-w-5xl mx-auto px-4 py-16 border-t border-[#e5e5e5]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-[#e5e5e5] border border-[#e5e5e5]">
          ...
          <div className="bg-white p-6 flex flex-col justify-between">
            <p className="font-arabic text-[#bbb] text-right" style={{fontSize:"22px",lineHeight:"2.6"}} dir="rtl">
              &#x648;&#x64E;&#x642;&#x64F;&#x644; &#x631;&#x651;&#x64E;&#x628;&#x651;&#x650; &#x632;&#x650;&#x62F;&#x652;&#x646;&#x650;&#x64A; &#x639;&#x650;&#x644;&#x652;&#x645;&#x64B;&#x627;
            </p>
            <div>
              <p className="text-[10px] text-[#aaa] text-right mt-4">Ta-Ha &middot; 20:114</p>
              <p className="text-[10px] text-[#aaa] text-right mt-1">My Lord, increase me in knowledge.</p>
            </div>
          </div>
        </div>
      </section>
